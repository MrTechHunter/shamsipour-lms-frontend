...
export const Container = styled.div`
  ...
  user-select: none;
  .highlight {
    background-color: ${variables.colors.green_50};
    border-radius: 4px;
    padding: 0 1px;
  }
`;
...
export const ArrowIcon = styled.span<IArrowIcon>`
  position: absolute;
  right: -20px;
  top: 5px;
  width: 18px;
...
interface ICheckBoxNodeContainer {
  hasCheckBox: boolean;
}
export const RightSide = styled.div<ICheckBoxNodeContainer>`
  height: fit-content;
  display: flex;
...
interface ISelectedNodeContainer {
  selectedNode: number | any;
}
export const NodeContainer = styled.div<ISelectedNodeContainer>`
  min-height: 20px;
  height: fit-content;
  display: flex;
