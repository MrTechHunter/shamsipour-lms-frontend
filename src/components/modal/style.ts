...
export const Container = styled.div<ContainerInterface>`
  z-index: 9999;
  padding: ${(props) => (props.type === 'delete' ? '28px 12px 12px 12px' : '12px')};
  text-align: justify;
  ${(props) => {
    if (props.size === 'sm') {
      return `
  ...
  display: flex;
  justify-content: ${(props) => (props.size === 'full-width' ? 'space-between' : 'space-around')};
...
export const Title = styled.div<TitleInterface>`
  ...
  text-align: right;
  color: ${(props) => (props.type === 'delete' ? `${variables.colors.black_87}` : `${variables.colors.blue_900}`)};
`;
...
export const Body = styled.div<BodyInterface>`
  ...
  line-height: 28px;
  display: flex;
  align-items: ${(props) => (props.type === 'delete' ? 'center' : 'flex-start')};
...
export const Circle = styled.div<CircleInterface>`
  ...
  height: 42px;
  border-radius: 50%;
  background-color: ${(props) => (props.type === 'delete' ? variables.colors.error : variables.colors.primary)};
