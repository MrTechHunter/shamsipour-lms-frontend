...
export const FilledFormField = styled.div<IField_IconWrapper>`
  ...
  border-bottom: 1px solid
    ${(props) => (props.error ? variables.colors.error : props.labelOnTop ? variables.colors.primary : 'unset')} !important;
  border-radius: 8px 8px 0 0;
  padding: 16px 0 8px 0;
  ${(props) => props.disabled && `background: ${variables.colors.white_100} !important`}
  &:hover {
    ${(props) => !props.labelOnTop && !props.disabled && `border-bottom: 1px solid ${variables.colors.black_87}`};
  }
  ${(props) =>
    !props.error &&
    props.isFilled &&
...
export const TinyInputWrapper = styled.input<IOutlinedInputWrapper>`
  width: ${(props) => {
    if (props.leftIcon && props.rightIcon) {
      ...
      return `${props.placeholderWidth + 0.7}px`;
    } else {
      return 'calc(100% - 0px)';
    }
  }};
...
export const TinyFormField = styled.div<IField_IconWrapper>`
  ...
  padding: 4px 8px;
  &:hover {
    ${(props) => !props.labelOnTop && !props.disabled && `border: 1px solid ${variables.colors.black_38} !important`};
  }
`;
...
export const TinyPlaceHolder = styled.div<IOutlinedInputWrapper>`
  width: ${(props) => (props.placeholderWidth ? `${props.placeholderWidth + 0.7}px` : 'max-content')};
  font-weight: 400;
  font-size: 12px;
  line-height: 20px;
  margin-left: 10px;
